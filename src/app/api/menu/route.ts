...
// Full menu data for auto-seeding
const MENU_DATA = [
  {
    name: "Chinese", slug: "chinese", icon: "🥡", order: 1,
    items: [
  ...
  {
    name: "Paneer", slug: "paneer", icon: "🧀", order: 2,
    items: [
  ...
  {
    name: "Sweet", slug: "sweet", icon: "🍯", order: 3,
    items: [
  ...
  {
    name: "Vegetable's", slug: "vegetables", icon: "🥦", order: 4,
    items: [
  ...
  {
    name: "Dal & Rice", slug: "dal-rice", icon: "🍚", order: 5,
    items: [
  ...
  {
    name: "Tandoor", slug: "tandoor", icon: "🫓", order: 6,
    items: [
  ...
  {
    name: "Fix Meal", slug: "fix-meal", icon: "🍽️", order: 7,
    items: [
  ...
  {
    name: "Dessert", slug: "dessert", icon: "🍨", order: 8,
    items: [
  ...
  {
    name: "Beverages", slug: "beverages", icon: "🥤", order: 9,
    items: [
      { name: "Mineral Water", price: 20, order: 1 },
      { name: "Butter Milk", price: 39, order: 2 },
      { name: "Masala Chaas", price: 49, order: 3 },
      { name: "Fresh Lime Soda", price: 59, order: 4 },
      { name: "Fresh Lime Water", price: 49, order: 5 },
    ]
  },
]
...
async function autoSeedMenu() {
  try {
      ...
      for (const item of cat.items) {
        await db.menuItem.create({
          data: {
            name: item.name,
            price: item.price,
            badge: (item as { badge?: string }).badge || null,
            variantTag: (item as { variantTag?: string }).variantTag || null,
    ...
    console.log('[Menu API] Menu auto-seeded successfully')
  } catch (seedError) {
    console.error('[Menu API] Auto-seed failed:', seedError)
  }
}
